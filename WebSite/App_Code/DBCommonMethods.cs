...
" left join HR_Designation on Employee.Rank=HR_Designation.Designation_ID where Status = 'Active'";
    ...
    public static DataTable GetEmployeeDesignationSelectIndexChange(string CNIC)
        {
            //  string Query = "select count(*) from tbl_Finger where UserID ='" + User_ID + "' ";
            string Query = @"SELECT Designation_ID,Department_ID, hdes.[designation] FROM [HR_Designation] hdes 
                            left join Employee emp on emp.Designation = hdes.Designation_ID 
                            left join mctx_Department md on emp.Department = md.Department_ID
                            where emp.Nic = '" + CNIC + "'";
                //"select * from Employee left join where  = '" + User_ID + "' and FingerIndex='" + fingerIndex + "'";
            //   Query += " where CNIC ='" + CNIC + "' ";

            // Query += " order by [ID] desc";
            DBManager ObjDBManager = new DBManager();
            string vmsConStr = string.Empty;
            //if (DBManager.isPrimary)
                vmsConStr = "vmsconnectionstring";
            //else
            //    vmsConStr = "vmsconnectionstringP";

            DataSet ds = ObjDBManager.SelectQuery(Query, "MaxUser", vmsConStr);
            DataTable dt;// = new DataTable();
            dt = ds.Tables["MaxUser"];
            return dt;
        }
...
//" and ('"+Date_From+"' IS NULL OR  cast(vlog.DATE  as date)>= CAST('"+Date_From+"' AS date) AND cast(vlog.DATE  as date) < DATEADD(DD,1,CAST('"+Date_To+"' AS DATE)) )";
    ...
    */
    ...
    //connection stirng method 22072022
    public static string GetActiveEmployees()
    {

        string Count = "";// new List<string>();
        using (SqlConnection conn = new SqlConnection())
        {
            conn.ConnectionString = ConfigurationManager
                    .ConnectionStrings["vmsconnectionstring"].ConnectionString;
            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.CommandText = "SELECT COUNT(*) as count FROM EMPLOYEE WHERE STATUS='Active' ";
                //cmd.Parameters.AddWithValue("@SearchText", prefix);
                cmd.Connection = conn;
                conn.Open();
                using (SqlDataReader sdr = cmd.ExecuteReader())
                {
                    while (sdr.Read())
                    {
                        Count = sdr["count"].ToString();//.Add(string.Format("{0}"));
                        //customers.Add(string.Format("{0}", sdr["EID"]));
                    }
                }
                conn.Close();
